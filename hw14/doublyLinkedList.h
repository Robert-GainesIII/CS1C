...
void doublyLinkedList<Type>::copyList(const doublyLinkedList<Type>& otherList)
{
   ...
   // nodeType<Type> *trailCurrent; //pointer just before current
    ...
    //found a nice solution here and figured it better than my own.
    if (otherList.first == nullptr)
        return;

    // Create a temp variable since ll.current doesn't move/change.
    nodeType<Type>* tempNode = otherList.first;

    // Allocate a new node in memory.
    first = new nodeType<Type>;
    // Copy over the value.
    first->value = tempNode->value;
    ...
    while (tempNode != nullptr)
    {
        ...
        current->next = new nodeType<Type>;
...
{
	doublyLinkedList<Type> temp(otherList);
